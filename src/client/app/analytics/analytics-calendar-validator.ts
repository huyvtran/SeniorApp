...
@Component({
  selector: 'analytics-calendar-error-messages',
...
export class AnalyticsCalendarValidatorComponent {
    ...
    for (let propertyName in this.control.errors) {
      ...
      try {
        if (this.control.errors.hasOwnProperty(propertyName) && this.control.touched) {
          let errorMsg = AnalyticsValidationService.getValidatorErrorMessage(propertyName,this.control.errors[propertyName]);
